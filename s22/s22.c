...
/// Partie 4
/*
// compare 2 BPDU : renvoi -1 si bpdu1 > bpdu2  / envoi 1 si bpdu1 < bpdu2  / sinon 0
int compareBPDU(BPDU bpdu1, BPDU bpdu2) {
    // Comparer l'identifiant de la racine
    int cmp = memcmp(&bpdu1.root_id, &bpdu2.root_id, sizeof(MACAddress));
    if (cmp != 0) {
        return cmp;
    }
    // Comparer le coût du chemin vers la racine
    if (bpdu1.cost < bpdu2.cost) {
        return -1;
    }
    if (bpdu1.cost > bpdu2.cost) {
        return 1;
    }
    // Comparer l'identifiant du pont émetteur
    return memcmp(&bpdu1.transmitting_id, &bpdu2.transmitting_id, sizeof(MACAddress));
}


int envoyerBPDU(Switch *source, Switch *destination) 
{
    // Vérifier si la BPDU de destination est moins bien que la BPDU de source
    if (compareBPDU(destination->bpdu, source->bpdu) < 0) {
        // si destination.bpdu < source.bpdu
        // Mettre à jour la BPDU de destination avec la BPDU de source
        destination->bpdu = source->bpdu;
        // Mettre à jour l'état du port correspondant dans le tableau PortState de destination
        int port = trouverPortPourMAC(destination, source->mac);
        if (port != -1) {
            destination->port_states[port] = PORT_DESIGNATED;
        }
        return 1; // Indiquer qu'un changement a été effectué
    }
    return 0; // Indiquer qu'aucun changement n'a été effectué
}

void mettreAJourEtatsPorts(Switch* sw) 
{
    // Parcourir tous les ports du sw
    for (int i = 0; i < sw->nbport; i++) {
        // Récupérer l'entrée de la table de commutation correspondante
        switchTableEntry *entry = &sw->table.tab_entrees[i];
        // Vérifier si le port est le port racine
        if (compareBPDU(entry->bpdu, sw->bpdu) == 0) {
            sw->port_states[i] = PORT_ROOT;
        }
        // Sinon, vérifier si le port est le port désigné
        else if (compareBPDU(entry->bpdu, sw->bpdu) > 0) {
            sw->port_states[i] = PORT_DESIGNATED;
        }
        // Sinon, le port est bloqué
        else {
            sw->port_states[i] = PORT_BLOCKED;
        }
    }
}


void stpProtocol(reseau *r) {
    int change = 1;
    while (change != 0) {
        change = 0;
        // Parcourir tous les switchs du réseau
        for (size_t i = 0; i < r->nb_equipements; i++) {
            if (r->tab_equipements[i].type == SWITCH) {
                Switch *sw = &r->tab_equipements[i].data.m_switch;
                // Récupérer les voisins du sw
                sommet voisins[r->m_graphe.ordre];
                size_t nb_voisins = sommets_adjacents(&r->m_graphe, i, voisins);
                // Parcourir tous les voisins du sw
                for (size_t j = 0; j < nb_voisins; j++) {
                    if (r->tab_equipements[voisins[j]].type == SWITCH) {
                        Switch *voisin = &r->tab_equipements[voisins[j]].data.m_switch;
                        // Envoyer la BPDU du sw à son voisin
                        change += envoyerBPDU(sw, voisin);
                    }
                }
                // Mettre à jour les états des ports du sw
                mettreAJourEtatsPorts(sw);
            }
        }
    }
}
